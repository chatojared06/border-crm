...
// 1. Agregamos la interfaz para que TypeScript sepa cómo es un Lead
interface Lead {
  id: number;
  name: string;
  email: string;
...
export default function EditLeadPage() {
  ...
  const { id } = useParams(); // Esto saca el ID de la URL
  ...
  const [formData, setFormData] = useState({
    ...
    phone: "",
    source: "Web",
    status: "NUEVO" // Agregamos el estado para poder cambiarlo
  });

  // Este useEffect busca los datos actuales del prospecto cuando entras a la página
  useEffect(() => {
    fetch("http://localhost:5000/api/leads")
      ...
      .then((datos) => {
        ...
        if (leadActual) {
          // Si lo encuentra, pre-llena el formulario
          setFormData({
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      // Axios lanza error si el status no es 2xx, por lo que si llegamos aquí, todo salió bien
      ...
      console.error(error);
      // Le decimos a TypeScript la forma del error de Axios
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button type="submit" className="w-full bg-blue-600 text-white px-4 py-3 rounded-lg hover:bg-blue-700 transition-colors mt-6">
            Guardar Cambios
          </button>
        </form>
      </div>
